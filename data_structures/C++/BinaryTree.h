...
enum BinaryTreeErrors { KEY_ERROR = 4 };
...
class BinaryTree {
  private:
    ...
    /*VS позволяет сделать std::vector<T>& res=std::vector<T>{}, однако в CLION
    это не работает как и в большинстве компиляторов, так как std::vector<T>{} -
    временный объект, который может быть удален, соответственно ссылки на него
    быть не может*/
  ...
  public:
    ...
    template <class T1>
    friend std::ostream& operator<<(std::ostream& os, const BinaryTree<T1>& tree);
...
template <class T>
BinaryTree<T>::BinaryTree(std::initializer_list<T> args) : _root {new BinaryNode(*args.begin())}, _size {1} {
...
typename BinaryTree<T>::BinaryNode* BinaryTree<T>::_remove(BinaryNode* cur_node, const T& val) {
    if (cur_node == nullptr)
        throw BinaryTreeException(BinaryTreeErrors::KEY_ERROR);
...
void BinaryTree<T>::_toVector(BinaryNode* cur_node, std::vector<T>& res) const {
    ...
    if (cur_node->left != nullptr)
        _toVector(cur_node->left, res);
    res.push_back(cur_node->val);
    if (cur_node->right != nullptr)
        _toVector(cur_node->right, res);
}
